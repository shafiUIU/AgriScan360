...
class AIClassifier:
    ...
    def _rule_classify(self, rgb_images, uv_images, gas_delta, rot_suspicion) -> dict:
        """
        UPDATE1 RUSHED: Dual-spectral vision fusion (RGB surface + 365nm UV fluorescence).
        Pillar 1 (RGB surface)      — ACTIVE (45° angles × 8)
        Pillar 2 (UV fluorescence)  — ACTIVE (45° angles × 8)
        Pillar 3 (Gas VOC)          — COMMENTED OUT (BME688 removed in this build)
        """
        # Analyse all 8 angles for RGB and UV
        rgb_feats = [ImageAnalyzer.analyze_rgb(img) for img in rgb_images]
        uv_feats  = [ImageAnalyzer.analyze_uv(img)  for img in uv_images] if uv_images else []
        ...
        p2_score = agg["uv_fluorescence_score"] if uv_feats else 0.0  # UV fluorescence

        # ── REMOVED: Pillar 3 Gas VOC ─────────────────────────────────────────
        # p3_score = self._gas_score(gas_delta)
        p3_score = 0.0   # Not used — BME688 absent

        # Weighted fusion: 60% RGB surface + 40% UV fungal fluorescence (when UV present)
        if uv_feats:
            fused = p1_score * 0.60 + p2_score * 0.40
        else:
            fused = p1_score
        ...
        reasons = []
        if p1_score > 0.4:
            reasons.append(f"Surface decay detected ({agg.get('rgb_brown_ratio',0)*100:.0f}% browning, {agg.get('rgb_dark_ratio',0)*100:.0f}% dark spots)")
        else:
            reasons.append(f"Surface skin intact ({100 - agg.get('rgb_brown_ratio',0)*100:.0f}% clean)")

        if uv_feats:
            if p2_score > 0.35:
                reasons.append(f"UV-A fluorescence revealed active fungal mould colonies (index: {p2_score:.2f})")
            else:
                reasons.append("no fungal fluorescence under 365nm UV-A")

        reason = "; ".join(reasons) + ". (Gas sensing disabled in Update1 Rushed)."

        return {
            "status":      status,
            "confidence":  confidence,
            "reason":      reason,
            "model_used":  "rule_based_vision_dual_v1",
            "pillar_scores": {
